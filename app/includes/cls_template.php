...
class cls_template
{
    ...
    var $cache_dir      = '';
    var $compile_dir    = '';
    var $cache_lifetime = FILE_CACHE_TIME; // 缓存更新时间, 默认 3600 秒
    ...
    function get_display($filename, $cache_id = '')
    {
        //error_reporting(E_ALL ^ E_NOTICE);
        $out = $this->fetch($filename, $cache_id);
            ...
            else
            {
                ...
                else
                {
                    ...
                    if ($cache_id)
                    {
                        ...
                        $data = serialize(array('template' => $this->template, 'expires' => $this->_nowtime + FILE_CACHE_TIME, 'maketime' => $this->_nowtime));
                        ...
                        if(defined('VERBOSE')){
                            echo "cache file: ".$hash_dir . '/' . $cachename . '.php';
                        }
                        $this->template = array();
                    }
                }
    ...
    {
        $compile_file_name = $this->compile_dir . '/' . basename($filename) . '.php';
 
        if (file_exists($compile_file_name)){
            $compilefilestat = @stat($compile_file_name);
            $compilefiletime  = $compilefilestat['mtime']; 
     
            $originalfilestat = @stat($filename);
            $originalfiletime  = $originalfilestat['mtime']; 
             
            if ($originalfiletime <= $compilefiletime)
            {
                $source = $this->_require($compile_file_name);
                return $source;
            }
        }
        if(defined('VERBOSE')){
        ...
        if (file_put_contents($compile_file_name, $source, LOCK_EX) === false)
        {
    ...
    function fetch_str($source)
    {
        ...
        //echo "<hr>",htmlspecialchars("after complie5:$source<hr>"),"<hr>";
        /*if(preg_match_all('~(<\?(?:\w+|=)?|\?>|language\s*=\s*[\"\']?php[\"\']?)~is', $source, $sp_match))
        {
         ...
         //echo "<hr>",htmlspecialchars("after complie3:$source<hr>"),"<hr>";
         return $source;
    }
    ...
    function is_cached($filename, $cache_id = '')
    {
        ...
        {
            $hash_dir = $this->cache_dir . '/' . substr(md5($cachename), 0, 1);
            //echo " get cache file ".$hash_dir . '/' . $cachename . '.php';
            if ($data = @file_get_contents($hash_dir . '/' . $cachename . '.php'))
            {
                ...
                if ($para === false || $this->_nowtime > FILE_CACHE_TIME+$para['maketime'])
                {
                    $this->caching = false;
    ...
    {
        //echo "<hr>",htmlspecialchars("before complie:$source<hr>");
        ...
        if ($file_type == '.dwt')
        {
            ...
            //echo "<hr>",htmlspecialchars("after complie:$source"),"<hr>";
        }
        ...
        //echo "<hr>",htmlspecialchars("after2 complie:$source"),"<hr>";
        return  $source;
    }
    ...
    {
        //echo htmlspecialchars('_eval(?' . '>' . trim($content));
        ob_start();
        eval('?' . '>' . trim($content));
