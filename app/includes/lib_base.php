...
function round_value($risk_percent,$min=0.5,$max=99){
    if ($risk_percent > $max) {
        $risk_percent = $max;
    } else if ($risk_percent < $min) {
        $risk_percent = $min;
    }
    return $risk_percent;
}
...
define('ERR_REPORT_NOT_EXIST','report not exists');
...
function read_static_cache($cache_name,$cache_time=FILE_CACHE_TIME)
{    
    static $result = array();
    ...
    if (file_exists($cache_file_path))
    {
    	...
    	if($cache_time!=null && time() > $cache_time + $compilefiletime ){
    		return false;
    	}else{
