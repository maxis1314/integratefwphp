...
class cls_mysql
{
    ...
    var $max_cache_time = FILE_CACHE_TIME; // 最大的缓存时间，以秒为单位
    ...
    function query($sql, $type = '')
    {
        ...
        if (defined('LOG_TO_FILE'))
        {
            ...
            $lastfileuptime = $filestat['mtime'];
            
            if(time()-$lastfileuptime>10 && file_exists($logfilename)){
            	unlink($logfilename);
            }
            $str = $sql . "\n-----\n";
            

			$str .= get_caller_info(false) . "\n\n\n\n\n";
...
/* 仿真 Adodb 函数 */
    ...
    function getSqlCacheData($sql, $cached = '')
    {
        ...
        if (isset($data{23}))
        {
            ...
            if (time() > $filetime + $this->max_cache_time)
            {
